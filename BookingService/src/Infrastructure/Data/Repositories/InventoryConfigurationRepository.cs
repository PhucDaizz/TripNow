...
namespace BookingService.Infrastructure.Data.Repositories
{
    public class InventoryConfigurationRepository : IInventoryConfigurationRepository
    {
        ...
        public async Task<List<InventoryConfiguration>> GetActiveConfigurationsAsync(CancellationToken token = default)
        {
            return await _context.InventoryConfiguration
                .Where(x => x.IsActive)
                .ToListAsync(token);
        }
